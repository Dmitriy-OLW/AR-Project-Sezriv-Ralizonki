...
namespace UnityVolumeRendering
{
    ...
    public class EditVolumeGUI : MonoBehaviour
    {
        ...
        public int selectedRenderModeIndex = 0;
        ...
        public void Render_Mode_Update(int index)
        {
            selectedRenderModeIndex = index;
            targetObject.SetRenderMode((RenderMode)selectedRenderModeIndex);

        }

        public void Visibility_Update(float x, float y)
        {
            Vector2 visibilityWindow = targetObject.GetVisibilityWindow();
            visibilityWindow.x = x;
            visibilityWindow.y = y;
            targetObject.SetVisibilityWindow(visibilityWindow);
        }

        public void Rotation_Update(float x, float y, float z)
        {
            rotation.x = x;
            rotation.y = y;
            rotation.z = z;
